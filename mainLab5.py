...
def swap_letters(password):
    ...
    new_password = password[-2:] + password[2:-2] + password[:2]
    print("Your swapped version of password: " + new_password)
    return new_password


def rep_with_upper(password):
    new_password = list(password)
    x = random.randint(0, len(new_password) - 1)
    new_password[x] = new_password[x].upper()
    new_password = ''.join(new_password)
    print("Your uppercased version of password: " + new_password)
    return new_password


def passGen(usersnumber):
    while usersnumber < 3 or usersnumber > 7:
        usersnumber = int(input("Please enter a value between 3 and 7: "))
    password_list = random.sample(words, usersnumber)
    password = ''.join(password_list)
    print("Your password is: " + password)
    return password


def search_letter(password, letter):

    if letter in password:
        print("The letter '" + letter + "' is present in the password.")
        return True
    else:
        print("The letter '" + letter + "' is not present in the password.")
        return False
